...
//import edu.umn.natsrl.vissimctrl.MeterLight;
...
//    private MeterLight currentRightLight = MeterLight.GREEN;
    ...
    public SimMeter(RampMeter meter) {
        this.meter = meter;
        this.id = meter.getId();
    ...
    public void updateMeterLight(VISSIMController updater) {
        ...
        if (isRedtimeUpdated && redTime == 0) {
            ...
            System.out.println("  - " + this.id + "'s red time is set ("+meterType+") : " + redTime);
            return;
        }
        ...
        if(cursor == 0) {
            // update rate
            if (isRedtimeUpdated) {
                System.out.println("  - " + this.id + "'s red time is updated("+meterType+") : " + redTime + " -> " + nextRedTime);
                this.redTime = this.nextRedTime;
                this.isRedtimeUpdated = false;
                this.redUpdatedTime = this.currentTime;
            }
    ...
    public void setMeterType(MeterType meterType) {
        ...
        if(meterType.isDual()) {
            meterId = new String[]{this.id + "_L", this.id + "_R"};
        } else {
            meterId = new String[]{this.id};
        }
    }
