...
public class SectionHelper {
    ...
    public EntranceState getEntrance(int detector_id) {
        for(EntranceState es : this.entranceStates) {
            for(Detector d : es.rnode.getDetectors()) {
                if(d.getDetectorId() == detector_id) return es;
            }
        }
        return null;
    ...
    public class ExitState extends State {
        ...
        public double[] getRampExitFlow() {            
            Detector[] detectors = rnode.getDetectors(new IDetectorChecker() {
                public boolean check(Detector d) {
                    if(d.getLaneType().isOffRamp()) return true;
                    else return false;
                }
            });
